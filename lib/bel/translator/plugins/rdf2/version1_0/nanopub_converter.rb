    ...
    module Version1_0
      class NanopubConverter
        ...
        def convert(nanopub)
          ...
          graph            = RDF::Graph.new
          graph           << s(resource, RDF.type, BELV1_0.Nanopub)
        ...
        def bel_statement(statement, nr, ng)
          ...
          ng << sg
          ng << s(nr, BELV1_0.hasStatement, stmt_uri)
        end

        def citation(citation, nr, ng)
          type = citation.type
          if type && (type.to_s.downcase == 'pubmed')
            ng << s(nr, BELV1_0.hasCitation, PUBMED[citation.id.to_s])
          end
        end

        def support(support, nr, ng)
          unless support.value.nil?
            ng << s(nr, BELV1_0.hasSupport, support.to_s)
          end
        end
        ...
        def bel_version(bel_version, nr, ng)
          ...
          ng << s(bel, RDF.type, BELV1_0.BiologicalExpressionLanguage)
          ng << s(bel, BELV1_0.hasVersion, spec.version.to_s)
          ng << s(nr, BELV1_0.hasBiologicalExpressionLanguage, bel)
        end
