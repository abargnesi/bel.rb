...
module BEL
  module BELRDF
    module Version1_0
      class TermConverter
        include BELParser::Language::Version1_0::Functions
        ...
        def convert(term)
          ...
          tg            = RDF::Graph.new
          tg           << s(term_uri, RDF.type, BELV1_0.Term)
        ...
        def handle_special_inner(outer_term, outer_uri, inner_term, tg)
          ...
          case
          when inner_function.return_type.subtype_of?(BELParser::Language::Version1_0::ReturnTypes::Abundance)
        ...
        def handle_activity_abundance(outer_term, outer_uri, inner_term, tg)
          if outer_term.function == Activity
            ...
            tg << s(outer_uri, BELV1_0.hasActivityAbundance, inner_uri)
          end
        end
        ...
        def handle_fragment(outer_term, outer_uri, inner_term, tg)
          if outer_term.function == ProteinAbundance
            ...
            if frag_range.is_a?(BELParser::Expression::Model::Parameter)
              tg << s(outer_uri, BELV1_0.hasFragmentRange, unquote(frag_range.to_s))
            end
            if frag_desc.is_a?(BELParser::Expression::Model::Parameter)
              tg << s(outer_uri, BELV1_0.hasFragmentDescriptor, unquote(frag_desc.to_s))
            end
          end
        end
        ...
        def handle_location(outer_term, outer_uri, inner_term, tg)
          ...
          if match
            ...
            if location.is_a?(BELParser::Expression::Model::Parameter)
              ...
              if param_uri
                tg << paramg
                tg << s(outer_uri, BELV1_0.hasLocation, param_uri)
              end
            end
          end
        end
        ...
        def handle_from_location(outer_term, outer_uri, inner_term, tg)
          if outer_term.function == Translocation
            ...
            if location.is_a?(BELParser::Expression::Model::Parameter)
              ...
              if param_uri
                tg << paramg
                tg << s(outer_uri, BELV1_0.hasFromLocation, param_uri)
              end
            end
          end
        end
        ...
        def handle_to_location(outer_term, outer_uri, inner_term, tg)
          if outer_term.function == Translocation
            ...
            if location.is_a?(BELParser::Expression::Model::Parameter)
              ...
              if param_uri
                tg << paramg
                tg << s(outer_uri, BELV1_0.hasToLocation, param_uri)
              end
            end
          end
        end
        ...
        def handle_molecular_activity(outer_term, outer_uri, inner_term, tg)
          if outer_term.function == Activity
            ...
            if ma.is_a?(BELParser::Expression::Model::Parameter)
              ...
              if param_uri
                tg << paramg
                tg << s(outer_uri, BELV1_0.hasActivityType, param_uri)
              end
            end
          end
        end
          ...
          if outer_term.function == Reaction
            inner_term.arguments.each do |arg|
              if arg.is_a?(BELParser::Expression::Model::Term)
                ...
                tg << s(outer_uri, BELV1_0.hasProduct, inner_uri)
              end
            end
          end
        end
        ...
        def handle_reactants(outer_term, outer_uri, inner_term, tg)
          if outer_term.function == Reaction
            inner_term.arguments.each do |arg|
              if arg.is_a?(BELParser::Expression::Model::Term)
                _, inner_uri, _ = convert(arg)
                tg << s(outer_uri, BELV1_0.hasReactant, inner_uri)
              end
            end
          end
        end
        ...
        def handle_pmod(outer_term, outer_uri, inner_term, tg)
          if outer_term.function == ProteinAbundance
            tg << s(outer_uri, RDF.type, BELV1_0.ModifiedProteinAbundance)
            tg << s(outer_uri, BELV1_0.hasModifiedProteinAbundance, outer_uri)

            mod_protein_uri = BELR[URI::encode(to_path_part(inner_term))]
            tg << s(outer_uri, BELV1_0.hasProteinModification, mod_protein_uri)
            ...
            if modtype && modtype.is_a?(BELParser::Expression::Model::Parameter)
              ...
              if param_uri
                tg << paramg
                tg << s(mod_protein_uri, BELV1_0.hasProteinModificationType, param_uri)
              end
            end

            if amino_acid && amino_acid.is_a?(BELParser::Expression::Model::Parameter)
              tg << s(mod_protein_uri, BELV1_0.hasAminoAcid, unquote(amino_acid.to_s))
            end

            if residue && residue.is_a?(BELParser::Expression::Model::Parameter)
              tg << s(mod_protein_uri, BELV1_0.hasProteinResidue, residue.to_s.to_i)
            end
          end
        end
        ...
        def handle_variant(outer_term, outer_uri, inner_term, tg)
          ...
          if match
            ...
            if hgvs_descriptor.is_a?(BELParser::Expression::Model::Parameter)
              tg << s(outer_uri, BELV1_0.hasVariant, unquote(hgvs_descriptor.to_s))
            end
          end
        end
        ...
        FUNCTION_HASH = {
          Abundance => BELV1_0.Abundance,
          Activity  => BELV1_0.Activity,
          BiologicalProcess => BELV1_0.BiologicalProcess,
          CellSecretion => BELV1_0.CellSecretion,
          CellSurfaceExpression => BELV1_0.CellSurfaceExpression,
          ComplexAbundance => BELV1_0.ComplexAbundance,
          CompositeAbundance => BELV1_0.CompositeAbundance,
          Degradation => BELV1_0.Degradation,
          Fusion => BELV1_0.FusionAbundance,
          GeneAbundance => BELV1_0.GeneAbundance,
          MicroRNAAbundance => BELV1_0.MicroRNAAbundance,
          Pathology => BELV1_0.Pathology,
          ProteinAbundance => BELV1_0.ProteinAbundance,
          Reaction => BELV1_0.Reaction,
          RNAAbundance => BELV1_0.RNAAbundance,
          Translocation => BELV1_0.Translocation
        }.freeze
